...
$categories = [
    'Service', 'Logiciel', 'Matériel', 'Consultation', 
    'Formation', 'Maintenance', 'Autre'
];
...
try {
    $stmt = $pdo->prepare("SELECT * FROM produits WHERE id = :id AND user_id = :user_id");
    ...
    $errors['general'] = __("produit_edit.error_retrieving") . $e->getMessage();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($produit_data['nom'])) {
        $errors['nom'] = __("produit_edit.name_required");
    }
    
    if (empty($produit_data['prix']) || !is_numeric($produit_data['prix']) || $produit_data['prix'] <= 0) {
        $errors['prix'] = __("produit_edit.price_invalid");
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $file_path)) {
                if ($produit_data['image_path'] && file_exists('uploads/' . $produit_data['image_path'])) {
                    unlink('uploads/' . $produit_data['image_path']);
                }
                $produit_data['image_path'] = 'products/' . $file_name;
            } else {
                $errors['image'] = __("produit_edit.upload_error");
            }
        } else {
            $errors['image'] = __("produit_edit.file_type_error");
        }
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $errors['general'] = __("produit_edit.update_error") . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title><?= __('produit_edit.title') ?> - efacture-maroc.com</title>
    ...
    <main class="flex-grow py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-2xl font-bold text-gray-900"><?= __('produit_edit.edit_product', ['name' => htmlspecialchars($produit['nom'])]) ?></h1>
                <a href="produits.php" class="text-primary hover:text-green-800 font-medium">
                    <i class="fas fa-arrow-left mr-1"></i> <?= __('produit_edit.back_to_list') ?>
                </a>
            </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-green-800">
                                <?= __('produit_edit.success_message') ?> 
                                <a href="produits.php" class="text-green-700 hover:text-green-600 underline"><?= __('produit_edit.view_list') ?></a>
                            </h3>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="nom" class="block text-sm font-medium text-gray-700 mb-1">
                                <?= __('produit_edit.name_label') ?> <span class="text-red-500">*</span>
                            </label>
                        ...
                        <div>
                            <label for="prix" class="block text-sm font-medium text-gray-700 mb-1">
                                <?= __('produit_edit.price_label') ?> <span class="text-red-500">*</span>
                            </label>
                        ...
                        <div>
                            <label for="categorie" class="block text-sm font-medium text-gray-700 mb-1">
                                <?= __('produit_edit.category_label') ?> <span class="text-red-500">*</span>
                            </label>
                            <select id="categorie" name="categorie" 
                                    class="form-input w-full rounded-md border-gray-300 shadow-sm">
                                <option value=""><?= __('produit_edit.select_category') ?></option>
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-1">
                                <?= __('produit_edit.new_image_label') ?>
                            </label>
                            ...
                            <?php endif; ?>
                            <p class="mt-1 text-xs text-gray-500"><?= __('produit_edit.image_keep_note') ?></p>
                        </div>
                        
                        <div class="md:col-span-2">
                            <label for="description" class="block text-sm font-medium text-gray-700 mb-1">
                                <?= __('produit_edit.description_label') ?>
                            </label>
                        ...
                        <?php if ($produit_data['image_path']): ?>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1"><?= __('produit_edit.current_image') ?></label>
                                <img src="uploads/<?= $produit_data['image_path'] ?>" alt="Image produit" class="image-preview mb-2">
